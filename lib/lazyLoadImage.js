...
LazyLoadImage.prototype = {
    ...
    filterDom: function () {
        return Array.from(document.querySelectorAll(`img[${this.opts.querySelect}]`));
    },
    imgLazyLoad: function () {
        const images = this.imagesQuery
        let imageItem
        ...
        return function(){ 
            ...
            for(let i=num; i<images.length; i++){
                ...
                if(isLoad){
                    const data_src = imageItem.getAttribute(this.opts.querySelect)
                    if(data_src){
                        imageItem.src = data_src
                        imageItem.removeAttribute(this.opts.querySelect)
                    }
                }
                num ++
            }
